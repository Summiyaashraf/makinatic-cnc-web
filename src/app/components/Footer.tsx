  ...
  return (
    <footer className={`bg-gray-900 text-gray-300 py-12 md:py-16 ${isAr ? 'text-right' : 'text-left'}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        {/* Grid: Mobile stack (1 col), Desktop 3 columns */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-12 border-b border-gray-800 pb-12">
          ...
          <div className="space-y-4">
            ...
            <p className={`text-sm leading-relaxed max-w-xs ${isAr ? 'mr-0 ml-auto' : ''}`}>
              {language === 'en' 
                ...
                : 'نقدم حلولاً متكاملة تتسم بالدقة والموثوقية والابتكار في مجال التصنيع الرقمي (CNC) بجميع أنحاء المملكة العربية السعودية.'}
            </p>
          </div>
          ...
          {/* Contact Info */}
          <div>
            <h4 className="text-white font-bold mb-6">{language === 'en' ? 'Contact Us' : 'تواصل معنا'}</h4>
            <ul className="space-y-4 text-sm">
              ...
              <li className={`flex items-center gap-3 ${isAr ? 'flex-row-reverse' : ''}`}>
                ...
                <span>{isAr ? 'المملكة العربية السعودية، جدة' : 'Saudi Arabia, Jeddah'}</span>
              </li>
            </ul>
          </div>
        ...
        <div className="pt-8 text-center text-[10px] md:text-xs text-gray-500 uppercase tracking-widest">
          <p>© {new Date().getFullYear()} Makinati CNC. {isAr ? 'جميع الحقوق محفوظة.' : 'All rights reserved.'}</p>
        </div>
      </div>
    </footer>
  );
